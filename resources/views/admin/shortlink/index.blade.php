      ...
      <tr>
        ...
        <td>
          <input
            ...
            readonly="readonly"
            value="http://www.trilce.edu.pe/l/{{$link->slug}}"
            style="width: 100%;"
          ><br>
          <a href="http://www.trilce.edu.pe/l/{{$link->slug}}" target="_blank">/l/{{$link->slug}}</a>
        </td>
        <td><a href="{{$link->url}}" target="_blank">{{$link->url}}</a></td>
        <td>{{$link->updated_at}}</td>
        <td>

          <form action="{{action('Admin\ShortlinkController@destroy', $link->id)}}" method="post" onsubmit="return secureDelete(this);">
          {{csrf_field()}}
          ...
          <a href="{{action('Admin\ShortlinkController@edit', $link->id)}}" class="btn btn-secondary btn-undefined" alt="Editar" role="button">

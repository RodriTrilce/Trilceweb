...
<body <?php body_class(); ?> >
	...
	<?php } ?>
		...
		<header class="wraper_header">
			...
			} elseif ( is_singular( 'event' ) || is_tax( 'event-category' ) ) {
				$event_page_info = get_page_by_path( 'event', OBJECT, 'page' );
				if ( $event_page_info ) {
					$event_page_id        = $event_page_info->ID;
					$rt_event_bannercheck = get_post_meta( $event_page_id, 'bannercheck', true );
				}	
			...
			if ( class_exists( 'Redux' ) ) {
				if (
						...
						in_array( $rt_event_bannercheck, array( 'bannerbreadcumbs', 'banneronly', 'breadcumbsonly', 'nobannerbreadcumbs' ), true ) ||

...
if ( ! function_exists( 'radiantthemes_global_var' ) ) {
	 ...
	 *
	 * @param  [type] $edusphere_opt_one   description.
	 * @param  [type] $edusphere_opt_two   description.
	 * @param  [type] $edusphere_opt_check description.
	...
	function radiantthemes_global_var(
		...
		global $edusphere_theme_option;
		if ( $edusphere_opt_check ) {
			if ( isset( $edusphere_theme_option[ $edusphere_opt_one ][ $edusphere_opt_two ] ) ) {
				return $edusphere_theme_option[ $edusphere_opt_one ][ $edusphere_opt_two ];
			}
		} else {
			if ( isset( $edusphere_theme_option[ $edusphere_opt_one ] ) ) {
				return $edusphere_theme_option[ $edusphere_opt_one ];
			}
		}
	}
}
...
if ( ! function_exists( 'radiantthemes_posted_on' ) ) :
	...
	function radiantthemes_posted_on() {
		...
		$byline = sprintf(
			...
			esc_html_x( '%s', 'post author', 'edusphere' ),
		...
		$published_on = sprintf(
			...
			'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark"><span data-hover="' . esc_html( get_the_date() ) . '">' . esc_html( get_the_date() ) . '</span></a>'
				...
				printf(
					...
					printf(
						wp_kses(
							'<span class="published-on"> ' . esc_html__( 'Posted on', 'edusphere' ) . ' ' . $published_on . '</span>
    						<span class="byline"> ' . esc_html__( 'By', 'edusphere' ) . ' ' . $byline . '</span>', 'post'
						)
					);
				...
				if ( is_single() ) {
					...
					if ( true == radiantthemes_global_var( 'display_categries', '', false ) ) :
						...
						if ( $categories_list && radiantthemes_categorized_blog() ) {
									printf(
										wp_kses( '<span class="category"><span class="ti-direction-alt"></span> ' . esc_html__( 'In', 'edusphere' ) . '', 'post' ) .
										/* translators: used between list items, there is a space after the comma */
										esc_html( ' %1$s' ) .
										wp_kses( '</span>', 'post' ),
...
function radiantthemes_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'edusphere_categories' ) ) ) {
		...
		set_transient( 'edusphere_categories', $all_the_cool_cats );
	}
...
function radiantthemes_category_transient_flusher() {
	...
	// Like, beat it. Dig?
	delete_transient( 'edusphere_categories' );
}

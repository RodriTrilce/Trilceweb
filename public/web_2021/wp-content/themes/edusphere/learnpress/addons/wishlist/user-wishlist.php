...
?>
	...
	<div id="learn-press-profile-tab-course-wishlist" class="<?php echo esc_attr( $has_courses ) ? 'has-courses' : ''; ?>">
		<?php if ( $has_courses ) { ?>
			<div class="learn-press-courses profile-courses courses-list learn-press-wishlist-courses">
				<?php foreach ( $wishlist as $post ) { ?>
					<?php learn_press_course_wishlist_template( 'wishlist-content.php' ); ?>
				<?php } ?>
			</div>
		<?php } ?>
		<?php learn_press_display_message( apply_filters( 'learn_press_wishlist_empty_course', __( 'No courses in your wishlist!', 'edusphere' ) ) ); ?>
	</div>
<?php
wp_reset_postdata();

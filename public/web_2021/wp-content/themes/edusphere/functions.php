...
require get_parent_theme_file_path( '/inc/learnpress/lp-template-hooks.php' );
...
if ( ! function_exists( 'radiantthemes_setup' ) ) :
	...
	function radiantthemes_setup() {
		 ...
		 * If you're building a theme based on edusphere, use a find and replace
		 * to change 'edusphere' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'edusphere', get_template_directory() . '/languages' );
		...
		*/
		...
		register_nav_menus(
			array(
				'top' => esc_html__( 'Primary', 'edusphere' ),
			)
		);
		...
		$edusphere_args = array(
			'default-color' => 'ffffff',
			'default-image' => '',
		);
		add_theme_support( 'custom-background', $edusphere_args );
		...
		function radiantthemes_editor_fonts_url() {
			...
			*/
			$jost = esc_html_x( 'on', 'Jost font: on or off', 'edusphere' );
			if ( 'off' !== $jost ) {
				$font_families = array();
				if ( 'off' !== $jost ) {
		...
		Redux::init('edusphere_theme_option');
		}
		/**
		 * Redux custom css
		 */
		function radiantthemes_custom_redux_css() {
			...
			function radiantthemes_override_css_fonts_url() {
				...
				*/
				if ( 'off' !== _x( 'on', 'Google font: on or off', 'edusphere' ) ) {
					$google_font_url = add_query_arg( 'family', rawurlencode( 'Jost:300,400,500,600,700' ), '//fonts.googleapis.com/css' );
				}
		...
		// This example assumes your opt_name is set to edusphere_theme_option, replace with your opt_name value.
		add_action( 'redux/page/edusphere_theme_option/enqueue', 'radiantthemes_custom_redux_css', 2 );
...
function radiantthemes_widgets_init() {
	...
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'edusphere' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'edusphere' ),
	...
	if ( class_exists( 'woocommerce' ) ) {
		register_sidebar(
			array(
				'name'          => esc_html__( 'Product | Sidebar', 'edusphere' ),
	...
	if ( class_exists( 'ReduxFrameworkPlugin' ) ) {
		// edusphere Footer Areas.
		for ( $j = 1; $j <= 4; $j++ ) {
			register_sidebar(
				array(
					'name'          => esc_html__( 'Footer | #', 'edusphere' ) . $j . '',
					'id'            => 'edusphere-footer-area-' . $j,
					'description'   => esc_html__( 'Add widgets here.', 'edusphere' ),
					'before_widget' => '<section id="%1$s" class="widget %2$s">',
					'after_widget'  => '</section>',
					'before_title'  => '<h4 class="widget-title">',
	...
	register_sidebar(
		array(
			'name'          => esc_html__( 'Hamburger | Sidebar', 'edusphere' ),
			'id'            => 'radiantthemes-hamburger-sidebar',
			'description'   => esc_html__( 'Add widgets for "Hamburger" menu from here. To turn it on/off please navigate to "Theme Options > Header" and select "Hamburger" for respetive header styles.', 'edusphere' ),
...
function radiantthemes_scripts() {
	...
	if ( ! class_exists( 'ReduxFrameworkPlugin' ) ) {
		...
		/**
		 * Load Jost Google Font when redux framework is not installed.
		 */
		function radiantthemes_default_google_fonts_url() {
			...
			*/
			if ( 'off' !== _x( 'on', 'Google font: on or off', 'edusphere' ) ) {
				$google_font_url = add_query_arg( 'family', rawurlencode( 'Jost:300,400,500,600,700|Lato:400' ), '//fonts.googleapis.com/css' );
			}
			return $google_font_url;
		}
		wp_enqueue_style(
	...
	wp_enqueue_style(
		'radiant-style-nav',
		get_parent_theme_file_uri( '/assets/css/radiant-style-nav.css' ),
		array(),
		time()
	);
	...
	wp_enqueue_style(
		...
		get_parent_theme_file_uri( '/assets/css/radiant-responsive-nav.css' ),
		array(),
		time()
	);
	wp_enqueue_style(
	...
	wp_enqueue_script(
		...
		array( 'jquery', 'radiantthemes-css-animate' ),
		time(),
		true
	);
	...
	wp_enqueue_script(
		'radiant-swiper',
		get_parent_theme_file_uri( '/assets/js/swiper-slider.js' ),
		array( 'swiper' ),
		time(),
		true
	);
...
$radiantthemes_theme_options = get_option( 'edusphere_theme_option' );
ob_start();
...
function radiantthemes_before_after_btn( $add_to_cart_html, $product, $args ) {
	$before = '<button class="radiantthemes-cart-border">'; // Some text or HTML here.
	$after  = '</button>'; // Add some text or HTML here as well.
	...
	function radiantthemes_pagination() {
		...
		$paginate_links = paginate_links(
			array(
				...
				'next_text' => '<span class="ti-angle-right"></span>',
				'prev_text' => '<span class="ti-angle-left"></span>',
				'end_size'  => 5,
				'mid_size'  => 5,
				'add_args'  => false,
				'type'      => 'list',
	...
	if ( have_posts() ) :
		...
		while ( have_posts() ) :
			...
			if ( is_category() ) {
				...
				if ( $style == 'one' || $style == 'two' || $style == 'three' || $style == 'four' ) {
					get_template_part( 'template-parts/content-lms-' . $style . '', get_post_format() );
				} elseif ( radiantthemes_global_var( 'course_style', '', false ) ) {
					get_template_part( 'template-parts/content-lms-' . radiantthemes_global_var( 'course_style', '', false ) . '', get_post_format() );
				} else {
					 get_template_part( 'template-parts/content-lms', get_post_format() );
				}
			}
...
function radiantthemes_load_posts_by_ajax_callback() {
	...
	<?php if ( $blog_posts->have_posts() ) : ?>
		...
		while ( $blog_posts->have_posts() ) :
			$blog_posts->the_post();
				get_template_part( 'template-parts/content-blog-' . radiantthemes_global_var( 'blog-style', '', false ) . '', get_post_format() );
		endwhile;
		?>
		<?php
	endif;
...
function radiantthemes_breadcrumbs() {
	...
	$home         = esc_html__( 'Home', 'edusphere' ); // text for the 'Home' link.
	...
	if ( is_home() && is_front_page() ) {
		...
		if ( 1 === $show_on_home ) {
			echo '<div id="crumbs"><a href="' . esc_url( $home_link ) . '">' . esc_html__( 'Home', 'edusphere' ) . '</a></div>';
		}
		...
		function radiantthemes_woocommerce_breadcrumbs() {
			...
			return array(
				...
				'before'      => '',
				'after'       => '',
				'home'        => _x( 'Home', 'breadcrumb', 'edusphere' ),
			);
		}
		...
		echo '<div id="crumbs"><a href="' . esc_url( $home_link ) . '">' . esc_html__( 'Home', 'edusphere' ) . '</a> ' . wp_kses( $delimiter, 'post' ) . ' ';
		if ( is_home() ) {
			echo wp_kses( $before, 'post' ) . esc_html( get_the_title( get_option( 'page_for_posts', true ) ) ) . wp_kses( $after, 'post' );
		} elseif ( is_tax( 'course_category' ) || is_post_type_archive( 'course_category' ) ) {
			echo esc_html__( 'Course category ', 'edusphere' ) . '' . wp_kses( $delimiter, 'post' );
			echo wp_kses( $before, 'post' ) . single_cat_title( '', false ) . '' . wp_kses( $after, 'post' );
		} elseif ( is_post_type_archive( 'lp_course' ) ) {
				echo wp_kses( $before, 'post' ) . esc_html__( 'All Courses', 'edusphere' ) . '' . wp_kses( $after, 'post' );
		} elseif ( is_category() ) {
			$this_cat = get_category( get_query_var( 'cat' ), false );
			if ( 0 != $this_cat->parent ) {
				echo wp_kses( get_category_parents( $this_cat->parent, true, ' ' . wp_kses( $delimiter, 'post' ) . ' ' ), 'post' );
			}
			echo wp_kses( $before, 'post' ) . esc_html__( 'Archive by category "', 'edusphere' ) . single_cat_title( '', false ) . '"' . wp_kses( $after, 'post' );
		} elseif ( is_search() ) {
			echo wp_kses( $before, 'post' ) . esc_html__( 'Search results for "', 'edusphere' ) . get_search_query() . '"' . wp_kses( $after, 'post' );
		} elseif ( is_day() ) {
			...
			echo wp_kses( $before, 'post' ) . esc_html__( 'Posts tagged "', 'edusphere' ) . single_tag_title( '', false ) . '"' . wp_kses( $after, 'post' );
		} elseif ( is_author() ) {
			global $author;
			$userdata = get_userdata( $author );
			echo wp_kses( $before, 'post' ) . esc_html__( 'Articles posted by ', 'edusphere' ) . esc_html( $userdata->display_name ) . wp_kses( $after, 'post' );
		} elseif ( is_404() ) {
			echo wp_kses( $before, 'post' ) . esc_html__( 'Error 404', 'edusphere' ) . wp_kses( $after, 'post' );
		}
		...
		if ( get_query_var( 'paged' ) ) {
			...
			echo esc_html_e( 'Page', 'edusphere' ) . ' ' . get_query_var( 'paged' );
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
				echo ')';
			}
		}
...
function radiantthemes_dashboard_submenu_page() {
	add_submenu_page(
		'themes.php',
		esc_html__( 'RadiantThemes Dashboard', 'edusphere' ),
		esc_html__( 'RadiantThemes Dashboard', 'edusphere' ),
		'manage_options',
...
function radiantthemes_plugins_submenu_page() {
	...
	add_submenu_page(
		'themes.php',
		esc_html__( 'Radiantthemes Install Plugins', 'edusphere' ),
		esc_html__( 'Radiantthemes Install Plugins', 'edusphere' ),
		'manage_options',
...
function radiantthemes_custom_fonts( $array ) {
	$theme_options = get_option( 'edusphere_theme_option' );
	$font_names    = array();
	for ( $i = 1; $i <= 50; $i++ ) {
	...
	$array = array(
		esc_html__( 'Custom Fonts', 'edusphere' ) => $final_custom_fonts,
	);
	return $array;
};

// add the filter.
add_filter( 'redux/edusphere_theme_option/field/typography/custom_fonts', 'radiantthemes_custom_fonts', 10, 1 );
...
function radiantthemes_add_themify_icons_tab( $tabs = array() ) {
	...
	$tabs['my-custom-icons'] = array(
		...
		'label'         => esc_html__( 'Themify Icons', 'edusphere' ),
		'labelIcon'     => 'ti ti-user',
		'prefix'        => 'ti-',
		'displayPrefix' => 'ti',
		'url'           => 'https://edusphere.radiantthemes.com/wp-content/themes/edusphere/assets/css/radiantthemes-all.min.css',
		'icons'         => $new_icons,
		'ver'           => '1.0.0',
	);
...
function radiantthemes_woo_pagination( $args ) {
	...
	$args['prev_text'] = '<span class="ti-angle-left"></span>';
	$args['next_text'] = '<span class="ti-angle-right"></span>';
...
function radiantthemes_fw_ext_backups_demos( $demos ) {
	$demos_array = array(
		'edusphere' => array(
			'title'        => __( 'edusphere', 'edusphere' ),
			'screenshot'   => 'https://edusphere.radiantthemes.com/wp-content/themes/edusphere/screenshot.png',
			'preview_link' => 'https://edusphere.radiantthemes.com/',
		),
	);

	$download_url = 'https://api.radiantthemes.com/demo-data/edusphere/';

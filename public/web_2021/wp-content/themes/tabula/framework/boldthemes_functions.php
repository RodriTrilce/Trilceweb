...
if ( ! function_exists( 'boldthemes_get_new_media_html' ) ) {
	function boldthemes_get_new_media_html( $arg = array() ) { 
		...
		if ( $video != '' ) {
			...
			$html = '<div class="btMediaBox btDarkSkin btLink"' . $bt_link_bg_image . '><blockquote><p><a href="' . esc_url( $link_url ) . '">' . wp_kses_post( $link_title ) . '</a></p><cite><a href="' . esc_url( $link_url ) . '">' . wp_kses_post( $link_url ) . '</a></cite></blockquote></div>';
	
		} else if ( $quote != '' ) {
			$bt_quote_bg_image = '';
			if ( $featured_image != '' ) $bt_quote_bg_image .= " style='background-image: url(" . esc_url( $featured_image ) . ")'";
			$html = wp_kses_post( $quote );
			if ( $permalink != '' ) $html = '<a href="' . esc_url( $permalink ) . '" title="' . esc_attr( $title ) . '">' . $html .'</a>';
			$html = '<div class="btMediaBox btQuote btDarkSkin"' . $bt_quote_bg_image . '><blockquote><p>' . $html . '</p><cite>' . wp_kses_post( $quote_author ) . '</cite></blockquote></div>';
...
if ( ! function_exists( 'boldthemes_pagination' ) ) {
	function boldthemes_pagination() {
		...
		$prev = get_previous_posts_link( esc_html__( 'Newer Posts', 'tabula' ) );
		$next = get_next_posts_link( esc_html__( 'Older Posts', 'tabula' ) );
...
if ( ! function_exists( 'boldthemes_theme_comment' ) ) {
	function boldthemes_theme_comment( $comment, $args, $depth ) {
		...
		<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
			<p><?php echo esc_html__( 'Pingback:', 'tabula' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( esc_html__( '(Edit)', 'tabula' ), '<span class="edit-link">', '</span>' ); ?></p>
		<?php
				break;
			default :
				...
				<div class="commentTxt">
					<div class="vcard divider">
						<?php
							...
							echo '<p class="posted">' . sprintf( esc_html__( '%1$s at %2$s', 'tabula' ), get_comment_date(), get_comment_time() ) . '</p>';
							if ( $rating > 0 && get_option( 'woocommerce_enable_review_rating' ) == 'yes' ) { ?>
								<div itemscope itemtype="http://schema.org/Rating" class="star-rating" title="<?php echo sprintf( esc_html__( 'Rated %d out of 5', 'tabula' ), $rating ) ?>">
									<span style="width:<?php echo wp_kses_post( ( $rating / 5 ) * 100 ); ?>%"><strong itemprop="ratingValue"><?php echo wp_kses_post( $rating ); ?></strong> <?php echo esc_html__( 'out of 5', 'tabula' ); ?></span>
								</div>
							<?php }
						?>
					</div>
					...
					<?php if ( '0' == $comment->comment_approved ) : ?>
						<p class="comment-awaiting-moderation"><?php echo esc_html__( 'Your comment is awaiting moderation.', 'tabula' ); ?></p>
					<?php endif; ?>
					...
					<div class="comment">
						...
						if ( comments_open() ) {
							echo '<p class="reply">';
								comment_reply_link( array_merge( $args, array( 'reply_text' => esc_html__( 'Reply', 'tabula' ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) );
							echo '</p>';
						}
						edit_comment_link( esc_html__( 'Edit', 'tabula' ), '<p class="edit-link">', '</p>' ); ?>
					</div>
				</div>
...
if ( ! function_exists( 'boldthemes_nav_menu' ) ) {
	function boldthemes_nav_menu( $walker = false, $theme_location = 'primary' ) {
		...
		$menu_depth = ( $theme_location == 'primary' ) ? 4 : 1;
		if ( $walker ) {
...
if ( ! function_exists( 'boldthemes_set_global_uri' ) ) {
	function boldthemes_set_global_uri() {
		...
		$data .= 'window.boldthemes_text.previous = \'' . esc_html__( 'previous', 'tabula' ) . '\';';
		$data .= 'window.boldthemes_text.next = \'' . esc_html__( 'next', 'tabula' ) . '\';';
		return $data;
	}
}
		...
		if ( ! is_404() && ! is_front_page() ) {
			...
			if ( ! $simple ) {
				...
				if ( ! is_singular() || is_page() ) {
					$output .= '<span><a href="' . esc_url( $home_link ) . '">' . esc_html__( 'Home', 'tabula' ) . '</a></span>';
				}
			} else {
				if ( ! is_singular() || is_page() ) {
					$output .= '<a href="' . esc_url( $home_link ) . '">' . esc_html__( 'Home', 'tabula' ) . '</a>';
				}
			}
			...
			if ( is_home() ) {
				...
				foreach( $ancestors as $ancestor ) {
					$output .= wp_kses_post( $item_prefix ) . '<a href="' . esc_url( get_permalink( $ancestor ) ) . '">' . wp_kses_post( get_the_title( $ancestor ) ) . '</a>' . wp_kses_post( $item_suffix );
				}
				...
				//if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) ) {
					...
					$sku = $product->get_sku() ? $product->get_sku() : esc_html__( 'N/A', 'tabula' );
					$subtitle = '<span class = "btProductSKU">' . esc_html__( 'SKU:', 'tabula' ) . ' ' . $sku . '</span>'; 
				...
				if ( ! is_null( boldthemes_get_option( 'pf_slug' ) ) && boldthemes_get_option( 'pf_slug' ) != '' ) {
					...
					$output .= $item_prefix . esc_html__( 'Portfolio', 'tabula' ) . $item_suffix;
				} else {
					$output .= $item_prefix . esc_html__( 'Portfolio', 'tabula' ) . $item_suffix;
				}
				...
				$output .= $item_prefix . esc_html__( 'Category', 'tabula' ) . $item_suffix;
				...
				$output .= $item_prefix . esc_html__( 'Tag', 'tabula' ) . $item_suffix;
				
				$subtitle = '';
		  
			} else if ( is_author() ) {
			
				$output .= $item_prefix . esc_html__( 'Author', 'tabula' ) . $item_suffix;
				...
				$output .= $item_prefix . esc_html__( 'Search', 'tabula' ) . $item_suffix;

...
{
    public $v_fields=array('distrito', 'provincia.provincia');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->select("$table.*  , provincia.provincia as id_provincia ");
        $this->db->from($table);
         $this->db->join("provincia", "distrito.id_provincia = provincia.id_provincia", "left");     
        $this->db->order_by("id_distrito","desc");
        $query = $this->db->get();
    ...
    function getRow($table, $id) {
        $this->db->select('*');
        $query = $this->db->get_where($table, array('id_distrito' => $id));
        $data = $query->result();
        return $data[0];
    }

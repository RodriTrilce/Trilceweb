...
class Usuario_marca_model extends CI_Model
{
    public $v_fields=array('marca.marca', 'usuario.usuario');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->select("$table.*  , marca.marca as id_marca  , usuario.usuario as id_usuario ");
        $this->db->from($table);
         $this->db->join("marca", "usuario_marca.id_marca = marca.id_marca", "left");  $this->db->join("usuario", "usuario_marca.id_usuario = usuario.id_usuario", "left");     
        $this->db->order_by("id_usuario_marca","desc");
        $query = $this->db->get();
    ...
    function getRow($table, $id) {
        $this->db->select('*');
        $query = $this->db->get_where($table, array('id_usuario_marca' => $id));
        $data = $query->result();
        return $data[0];
    }
    ...
    {
        $this->db->where("id_usuario_marca",$id);

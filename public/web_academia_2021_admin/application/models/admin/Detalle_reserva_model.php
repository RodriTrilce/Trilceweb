...
class Detalle_reserva_model extends CI_Model
{
    public $v_fields=array('reserva.id_reserva', 'servicio.servicio', 'precio');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->select("$table.*  , reserva.id_reserva as id_reserva  , servicio.servicio as id_servicio ");
        $this->db->from($table);
         $this->db->join("reserva", "detalle_reserva.id_reserva = reserva.id_reserva", "left");  $this->db->join("servicio", "detalle_reserva.id_servicio = servicio.id_servicio", "left");     
        $this->db->order_by("id_detalle_reserva","desc");
        $query = $this->db->get();
    ...
    function getRow($table, $id) {
        $this->db->select('*');
        $query = $this->db->get_where($table, array('id_detalle_reserva' => $id));
        $data = $query->result();
        return $data[0];
    }
    ...
    {
        $this->db->where("id_detalle_reserva",$id);

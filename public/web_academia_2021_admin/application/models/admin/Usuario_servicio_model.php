...
class Usuario_servicio_model extends CI_Model
{
    public $v_fields=array('usuario.usuario', 'servicio.servicio');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->select("$table.*  , usuario.usuario as id_usuario  , servicio.servicio as id_servicio ");
        $this->db->from($table);
         $this->db->join("usuario", "usuario_servicio.id_usuario = usuario.id_usuario", "left");  $this->db->join("servicio", "usuario_servicio.id_servicio = servicio.id_servicio", "left");     
        $this->db->order_by("id_usuario_servicio","desc");
        $query = $this->db->get();
    ...
    function getRow($table, $id) {
        $this->db->select('*');
        $query = $this->db->get_where($table, array('id_usuario_servicio' => $id));
        $data = $query->result();
        return $data[0];
    }
    ...
    {
        $this->db->where("id_usuario_servicio",$id);

...
{
    public $v_fields=array('servicio', 'descripcion', 'precio', 'tipo_servicio.tipo_servicio');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->select("$table.*  , tipo_servicio.tipo_servicio as id_tipo_servicio ");
        $this->db->from($table);
         $this->db->join("tipo_servicio", "servicio.id_tipo_servicio = tipo_servicio.id_tipo_servicio", "left");     
    ...
    function getRow($table, $id) {
        $this->db->select('*');
        $query = $this->db->get_where($table, array('id_servicio' => $id));
        $data = $query->result();
        return $data[0];
    }

...
class Usuario_detalle_model extends CI_Model

{

    public $v_fields=array('antecedentes_penales', 'estado.estado', 'verificacion_domociliaria', 'estado.estado', 'ruc_personal', 'estado.estado', 'entrevista', 'estado.estado', 'evaluacion_destrezas', 'estado.estado', 'corroborar_referencia', 'estado.estado', 'fecha_registro', 'fecha_actualizacion', 'usuario.usuario');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->select("$table.*  , est1.estado as estado_ap  , est2.estado as estado_vd  , est3.estado as estado_rp  , est4.estado as estado_e  , est5.estado as estado_ed  , est6.estado as estado_cr  , usuario.usuario as id_usuario ");

        $this->db->from($table);

         $this->db->join("estado as est1", "usuario_detalle.estado_ap   = est1.id_estado", "left"); 
          $this->db->join("estado as est2", "usuario_detalle.estado_vd  = est2.id_estado", "left");  
          $this->db->join("estado as est3", "usuario_detalle.estado_rp  = est3.id_estado", "left");  
          $this->db->join("estado as est4", "usuario_detalle.estado_e   = est4.id_estado", "left");  
          $this->db->join("estado as est5", "usuario_detalle.estado_ed  = est5.id_estado", "left");  
          $this->db->join("estado as est6", "usuario_detalle.estado_cr  = est6.id_estado", "left");  
          $this->db->join("usuario", "usuario_detalle.id_usuario = usuario.id_usuario", "left");     

        $this->db->order_by("id_usuario_detalle","desc");
    ...
    function getRow($table, $id) {
        ...
        $query = $this->db->get_where($table, array('id_usuario_detalle' => $id));
...

    {
        ...
        $this->db->where("id_usuario_detalle",$id);

...
class Usuario_distrito_atencion_model extends CI_Model
{
    public $v_fields=array('usuario.usuario', 'distrito.distrito');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->select("$table.*  , usuario.usuario as id_usuario  , distrito.distrito as id_distrito ");
        $this->db->from($table);
         $this->db->join("usuario", "usuario_distrito_atencion.id_usuario = usuario.id_usuario", "left");  $this->db->join("distrito", "usuario_distrito_atencion.id_distrito = distrito.id_distrito", "left");     
        $this->db->order_by("id_usuario_atencion","desc");
        $query = $this->db->get();
    ...
    function getRow($table, $id) {
        $this->db->select('*');
        $query = $this->db->get_where($table, array('id_usuario_atencion' => $id));
        $data = $query->result();
        return $data[0];
    }
    ...
    {
        $this->db->where("id_usuario_atencion",$id);

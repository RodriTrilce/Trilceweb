...
class Medio_pago_model extends CI_Model
{
    public $v_fields=array('medio_pago');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->order_by("id_medio_pago","desc");
        $query = $this->db->get();
    ...
    function getRow($table, $id) {
        $this->db->select('*');
        $query = $this->db->get_where($table, array('id_medio_pago' => $id));
        $data = $query->result();
        return $data[0];
    }

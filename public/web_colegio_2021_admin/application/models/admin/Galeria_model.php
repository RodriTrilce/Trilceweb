...
{
    public $v_fields=array('galeria', 'foto_cover', 'foto1', 'foto2', 'foto3', 'foto4', 'servicio.servicio');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->select("$table.*  , servicio.servicio as id_servicio ");
        $this->db->from($table);
         $this->db->join("servicio", "galeria.id_servicio = servicio.id_servicio", "left");     
    ...
    function getRow($table, $id) {
        $this->db->select('*');
        $query = $this->db->get_where($table, array('id_galeria' => $id));
        $data = $query->result();
        return $data[0];
    }

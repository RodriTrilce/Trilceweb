...
{
    public $v_fields=array('usuario', 'clave', 'email', 'nombres', 'apellidos', 'experiencia', 'anio', 'distrito.distrito', 'direccion', 'telefono', 'celular', 'perfil.perfil');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->select("$table.*  , distrito.distrito as id_distrito  , perfil.perfil as id_perfil ");
        $this->db->from($table);
         $this->db->join("distrito", "usuario.id_distrito = distrito.id_distrito", "left");  $this->db->join("perfil", "usuario.id_perfil = perfil.id_perfil", "left");     
    ...
    function getRow($table, $id) {
        $this->db->select('*');
        $query = $this->db->get_where($table, array('id_usuario' => $id));
        $data = $query->result();
        return $data[0];
    }

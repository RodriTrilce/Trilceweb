...
{
    public $v_fields=array('provincia', 'departamento.departamento');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->select("$table.*  , departamento.departamento as id_departamento ");
        $this->db->from($table);
         $this->db->join("departamento", "provincia.id_departamento = departamento.id_departamento", "left");     
    ...
    function getRow($table, $id) {
        $this->db->select('*');
        $query = $this->db->get_where($table, array('id_provincia' => $id));
        $data = $query->result();
        return $data[0];
    }

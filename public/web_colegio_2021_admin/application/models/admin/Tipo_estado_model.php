...
{
    public $v_fields=array('tipo_estado');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->order_by("id_tipo_estado","desc");
        $query = $this->db->get();
    ...
    function getRow($table, $id) {
        $this->db->select('*');
        $query = $this->db->get_where($table, array('id_tipo_estado' => $id));
        $data = $query->result();
        return $data[0];
    }
    ...
    {
        $this->db->where("id_tipo_estado",$id);

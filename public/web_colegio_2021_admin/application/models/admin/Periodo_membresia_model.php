...
class Periodo_membresia_model extends CI_Model
{
    public $v_fields=array('periodo');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->order_by("id_periodo_membresia","desc");
        $query = $this->db->get();
    ...
    function getRow($table, $id) {
        $this->db->select('*');
        $query = $this->db->get_where($table, array('id_periodo_membresia' => $id));
        $data = $query->result();
        return $data[0];
    }
    ...
    {
        $this->db->where("id_periodo_membresia",$id);

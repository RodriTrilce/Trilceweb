...
class Usuario_medio_pago_model extends CI_Model
{
    public $v_fields=array('medio_pago.medio_pago', 'usuario.usuario');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->select("$table.*  , medio_pago.medio_pago as id_medio_pago  , usuario.usuario as id_usuario ");
        $this->db->from($table);
         $this->db->join("medio_pago", "usuario_medio_pago.id_medio_pago = medio_pago.id_medio_pago", "left");  $this->db->join("usuario", "usuario_medio_pago.id_usuario = usuario.id_usuario", "left");     
        $this->db->order_by("id_usuario_medio_pago","desc");
        $query = $this->db->get();
    ...
    function getRow($table, $id) {
        $this->db->select('*');
        $query = $this->db->get_where($table, array('id_usuario_medio_pago' => $id));
        $data = $query->result();
        return $data[0];
    }
    ...
    {
        $this->db->where("id_usuario_medio_pago",$id);

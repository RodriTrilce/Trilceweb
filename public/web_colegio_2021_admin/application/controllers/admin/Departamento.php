...
class Departamento extends CI_Controller
{
	...
	{   
		...
		$this->form_validation->set_rules('departamento', 'Departamento Name', 'required');
        ...
        {
			$saveData['departamento'] = set_value('departamento');
	...
	function edit($id)
    {
	 ...
	 function delete($id='')
     {
		if (!$this->ion_auth->logged_in()) {
 		...
 		if(isset($id) and !empty($id))
		{
			$count=$this->departamento->getCount('departamento','departamento.id_departamento',$id);
			if(isset($count) and !empty($count))
			{
				$this->departamento->delete('departamento','id_departamento',$id);
			...
			for($a=0; $a<count($all_ids); $a++)
	  		{
	  			if($all_ids[$a]!="")
	  			{
					$this->departamento->delete('departamento','id_departamento',$all_ids[$a]);
